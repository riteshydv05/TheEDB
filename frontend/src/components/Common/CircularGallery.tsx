    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          vec3 p = position;
          p.z = (sin(p.x * 4.0 + uTime) * 0.5 + cos(p.y * 2.0 + uTime) * 0.5) * (0.05 + uSpeed * 0.2);
...
class App {
  ...
  onResize() {
    ...
    if (this.medias) {
      this.medias.forEach((media: Media) => media.onResize({ screen: this.screen, viewport: this.viewport }));
    }
  }
  update() {
    ...
    if (this.medias) {
      this.medias.forEach((media: Media) => media.update(this.scroll, direction));
    }
...
export default function CircularGallery({
  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      <button className="gallery-nav-button prev" onClick={handlePrev} aria-label="Previous">
        ‹
      </button>
      <button className="gallery-nav-button next" onClick={handleNext} aria-label="Next">
        ›
      </button>
    </div>
  );
